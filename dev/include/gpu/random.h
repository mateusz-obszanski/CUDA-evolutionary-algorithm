...
concept IsInitializableRndState =
    SameAsAny<State, curandStateXORWOW_t, curandStateMRG32k3a_t,
...
template <IsInitializableRndState State = curandState>
__global__ void
initialize_rnd_states(device_ptr_inout<State> states, const unsigned int length,
    ...
    curand_init(params.seed, params.first_seq + tid, params.offset,
                &states[tid]);
}
...
template <IsInitializableRndState State = curandState>
inline void
initialize_rnd_states(State* const states, std::size_t length,
                      const RndStateInitParams& params = RndStateInitParams{},
...
inline void
initialize_rnd_states(RndStateMemory<State, Allocator>& states,
                      const RndStateInitParams& params = RndStateInitParams{},
                      const cudaStream_t&       stream = 0) {
...
concept RndDistributionFunctor =
    IsInitializableRndState<State> and requires(F f, State s) {
        { f(s) } -> std::same_as<float>;
    };
...
template <typename IteratorOut, IsInitializableRndState State,

          template <typename TT> typename Allocator,

          functional::RndDistributionFunctor<State> G>
inline void
generate_distribution(IteratorOut begin, IteratorOut end,
                      RndStateMemory<State, Allocator>& states,
                      const cudaStream_t&               stream) {

    thrust::transform(thrust::device.on(stream), states.begin_thrust(),
                      states.begin_thrust() +
                          device::iterator::distance(begin, end),
                      begin, G{});
}

} // namespace utils

template <typename IteratorOut, IsInitializableRndState State = curandState,

          template <typename TT>
          typename Allocator = device::memory::allocator::DeviceAllocator>
inline void
uniform(IteratorOut begin, IteratorOut end,
        RndStateMemory<State, Allocator>& states,
        const cudaStream_t&               stream = 0) {

    utils::generate_distribution<IteratorOut, State, Allocator,
                                 functional::UniformGen<State>>(begin, end,
                                                                states, stream);
}
...
template <typename IterIn, typename IterOut,
    ...
    requires std::copyable<typename IterIn::value_type> and
             std::convertible_to<typename IterIn::value_type,
                                 typename IterOut::value_type>
inline IterOut
choose_with_prob_without_replacement(IterIn begin, IterIn end, IterOut out,
                                     RndStateMemory<State, Allocator>& states,
                                     const float                       p,
                                     const cudaStream_t stream = 0) {
    ...
    return thrust::copy_if(begin, end, choiceChances.begin(), out,
                           [=] __device__(const float pk) { return pk < p; });
}

template <typename IterOut, std::integral K = std::size_t,
          IsInitializableRndState State = curandState,

          template <typename TT>
          typename Allocator = device::memory::allocator::DeviceAllocator>
inline void
choose_k_without_replacement(const std::size_t n, IterOut out, K k,
                             RndStateMemory<State, Allocator>& states,
                             const cudaStream_t                stream = 0) {
    ...
    thrust::stable_sort_by_key(thrust::device.on(stream), priorities.begin(),
                               priorities.end(), indices.begin());

    thrust::copy_n(thrust::device.on(stream), indices.begin(), k, out);
}

template <typename IterIn, typename IterOut, std::integral K = std::size_t,
          IsInitializableRndState State = curandState,

          template <typename TT>
          typename Allocator = device::memory::allocator::DeviceAllocator>

    requires IterValConvertibleTo<IterIn, IterOut>
inline void
choose_k_without_replacement(IterIn begin, IterIn end, IterOut out, K k,
                             RndStateMemory<State, Allocator>& states,
                             const cudaStream_t                stream = 0) {
...
/// @param stream
template <typename IterOut, IsInitializableRndState State = curandState,
...
inline void
mask(IterOut begin, IterOut end, RndStateMemory<State, Allocator>& states,
     const float maskProbability, const cudaStream_t& stream = 0) {
...
inline void
shuffle_at(Iter begin, IdxIter indicesBegin, IdxIter indicesEnd,
           ThrustRndEngine& rng) {
    ...
    thrust::shuffle_copy(indicesBegin, indicesEnd, shuffledIndices.begin(),
                         rng);

    reordering::swap_sparse_n(begin, indicesBegin, shuffledIndices.begin(), n);
}

template <typename Iter, typename IterMask, typename ThrustRndEngine>
    requires ShuffleMaskedAble<Iter, IterMask>
inline void
shuffle_masked_n(Iter begin, const std::size_t n, IterMask beginMask,
                 ThrustRndEngine& rng) {
    ...
    const auto lastMaskIdx =
        mask::mask_indices_n(beginMask, n, maskIndices.begin());
...
template <typename Iter, typename ThrustRndEngine,
          ...
          typename Allocator = device::memory::allocator::DeviceAllocator>
inline void
shuffle_with_prob(Iter begin, Iter end, const float p,
                  RndStateMemory<State, Allocator>& states,
                  ThrustRndEngine& rng, const cudaStream_t stream = 0) {
    ...
    thrust::shuffle_copy(chosenIndices.begin(), endChosenIdx,
                         targetIndices.begin(), rng);

    reordering::swap_sparse_n(begin, chosenIndices.begin(),
                              targetIndices.begin(), nChosen, stream);
}
...
inline void
crossover(IterX beginX, IterX endX, IterY beginY, IterOut1 beginOut1,
          IterOut2 beginOut2, RndStateMemory<State, Allocator>& states,
          const cudaStream_t stream = 0) {
    ...
    combining::crossover(beginX, endX, beginY, swapChances.begin(),
                         functors::ThresholdLess{0.5}, beginOut1, beginOut2,
                         stream);
}
