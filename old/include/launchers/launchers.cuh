...
namespace launcher {
    ...
    fill(
        ...
        if (std::is_integral_v<T> and
            shorterThanByteOrZero(fillval) and
            mode == FillMode::ALLOW_MEMSET_SHORT_OR_ZERO)
        ...
        } catch (const cuda_utils::host::DeviceKernelLaunchError& err) {
            throw FillError(err.errCode);
        }
    }
    ...
    DEFINE_CUDA_ERROR(
        DeviceCopyError,
        "Could not copy data from device to device")
